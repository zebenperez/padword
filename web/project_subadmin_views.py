...
def paginate_projects(request, items):
    rows = request.session.get("projects_subadmin_rows", 10)
    page = request.session.get("projects_subadmin_page", 1)
...
def projects(request, company_id=None, project_id=None):
    try:
        if "projects_subadmin_page" not in request.session:
            request.session["projects_subadmin_page"] = 1
        if "projects_subadmin_rows" not in request.session:
            request.session["projects_subadmin_rows"] = 10
        ...
        return render(request, "web/projects-subadmin/projects.html", {
            ...
            'company': company,
            'page_url': 'projects-subadmin-page-rows',
            'active': 'projects',
        })
...
def projects_page_rows(request, page=1, rows=10):
    request.session["projects_subadmin_page"] = page
    request.session["projects_subadmin_rows"] = rows
    return redirect(request.META.get('HTTP_REFERER') or reverse('projects-subadmin'))
...
def project_user_refresh_token(request):
    ...
    return render(request, "web/projects-subadmin/project-form-token.html", {'obj': project, 'user_lock': user_lock,})
