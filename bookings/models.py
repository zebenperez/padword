    ...
    def to_tickets2(self, start_date="", end_date=""):
        if start_date != "":
            ...
            if end_date != "":
                ...
                fi_list = FormInstance.objects.filter(form_uuid=self.uuid, date__gte=s_date).order_by("-start_date")[:50]
        else:
            fi_list = FormInstance.objects.filter(form_uuid=self.uuid).order_by("-start_date")[:50]
        resp = {"tickets": []}
        for fi in fi_list:
