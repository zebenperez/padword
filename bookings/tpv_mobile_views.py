...
def tpv_access(request, project_uuid, mobile=""):
    ...
    if check_user(request.user):
        next_url = reverse("tpv-mob-index", kwargs = context)
    else:
        auth.logout(request)
        next_url = reverse("tpv-mob-login-form")
...
def tpv_login(request):
    ...
    try:
        ...
        if username == "" or password == "":
            ...
            return render(request, "bookings/tpv/mobile/tpv-form-login.html", {'project_uuid': project_uuid, 'error': err})
