...
def test_stripe(request):
    import stripe
    stripe.api_key = "sk_test_51PAwwh14EEiK5wo0fArBnn5kkPbri8PkDCTyiqcC2jknwqwYqjjSwHP8NQQmtVESzuIJ95TPukiN5m509SptMRAN00mmKFNDkW"

    if False:
        # Crear cliente
        customer = stripe.Customer.create(
                  name="Dani Diaz",
                    email="danidiaz@shidix.com",)
        print (customer["id"])

    if False:
        customer = stripe.Customer.retrieve("cus_Q9LsDjy8eXJZX6")
        print (customer)
        cvc_token = stripe.Token.create(cvc_update={"cvc": "123"})
        pm = stripe.PaymentMethod.retrieve("pm_1PJ3GL14EEiK5wo03ZnybHBz")
        intent = stripe.PaymentIntent.create(
          ...
          currency='eur',
          confirm=True,
          return_url="https://padword.shidix.es/bookings/test",
          automatic_payment_methods={ 'enabled': True, },
        )

    if True:
        customer = stripe.Customer.retrieve("cus_Q9LsDjy8eXJZX6")
#         stripe.PaymentIntent.confirm("pi_3PJ3OI14EEiK5wo01UAhjU5K",
#             payment_method="pm_1PJ3GL14EEiK5wo03ZnybHBz",
#             return_url="https://padword.shidix.es/bookings/test"
#         )
        pm = stripe.PaymentMethod.retrieve("pm_1PJ3GL14EEiK5wo03ZnybHBz")
        intent = stripe.PaymentIntent.create(
    ...
    return HttpResponse(f'{customer}<br>{intent}<br><hr>{pm}')
